...
export const users = pgTable("users", {
  ...
  smoochXp: integer("smooch_xp").notNull().default(0),
  ...
  smoochsSent: integer("smoochs_sent").notNull().default(0),
  ...
  smoochsReceived: integer("smoochs_received").notNull().default(0),
...
export const smoochInteractions = pgTable("smooch_interactions", {
  fromUserId: text("from_user_id").notNull().references(() => users.userId),
  toUserId: text("to_user_id").notNull().references(() => users.userId),
  count: integer("count").notNull().default(1),
  lastSmoochAt: timestamp("last_smooch_at").defaultNow(),
}, (table) => ({
  uniqueIdx: uniqueIndex("smooch_unique_idx").on(table.fromUserId, table.toUserId)
}));